<?php
    ...
    if (isset($_POST) & !empty($_POST)) {
        ...
        if ($innerPin=="" || $cmfir == "" || $firstName== ""  || $SIN=="" || $phoneN == ""|| $address =="" || $email == "") {
            ...
            //check uniqueness
            $sql = "select * from Labourer where innerPIN = '$innerPin'";
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
            if ($stmt->rowCount() > 0){
                  $message = "Inner Pin Already Exist, Please Select an Unique Inner Pin";
                  $success = 0;
            }
...
<body>
    ...
    <form method="post">
        ...
        <p>Please Input 4 Digit Numerical Inner Pin*:<br>
            <input class="short" type="password" name="innerPin" required>
        </p>
        <p>Confirm Inner Pin*:<br>
